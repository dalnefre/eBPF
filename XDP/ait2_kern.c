...
swap_mac_addrs(void *ethhdr)
{
    ...
    __u16 *my_mac = get_my_mac();
    if (my_mac && (*my_mac != 0)) {
        copy_mac_addr(eth + 0, eth + 3);
        copy_mac_addr(eth + 3, my_mac);
    } else {
...
#define DST_MAC_OFS   (0)                       // 0
#define SRC_MAC_OFS   (DST_MAC_OFS + ETH_ALEN)  // 6
#define ETH_TYPE_OFS  (SRC_MAC_OFS + ETH_ALEN)  // 12
#define MESSAGE_OFS   (DST_MAC_OFS + ETH_HLEN)  // 14
    ...
    if (data[MSG_LEN_OFS] == n_6) {  // live len = 6
        ...
        switch (b) {
            case INIT_STATE: {  // init
                void *my_mac = get_my_mac();
                if (my_mac
                &&  (cmp_mac_addr(my_mac, data + SRC_MAC_OFS) < 0)) {
                    live_msg_fmt(data, PONG_STATE);
#if TRACE_STAT
                    bpf_printk("I am Alice.\n");
#endif
                } else {
            ...
            case PONG_STATE: {  // pong
                ...
                if (data[STATE_OFS] == PING_STATE) {  // ping/pong
                    __u64 *p = acquire_ait();
                    if (p && (*p != AIT_EMPTY)) {  // outbound ait?
                        i = *p;
                        ait_msg_fmt(data, GOT_AIT_STATE);
